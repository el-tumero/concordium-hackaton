...
#[init(contract = "test", parameter="InitParameter")]
...
fn change<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    host: &mut impl HasHost<State, StateApiType = S>,
) -> Result<(), Error> {
    // Your code
    let param: ChangeParameter = ctx.parameter_cursor().get()?;

    host.state_mut().message = param.message;
...
#[receive(contract = "test", name = "view", return_value = "State")]
